...
while [ "${1}" != "" ]; do
    ...
    while [ "${!counter}" != "" ]; do
        if [ "${!counter}" == "$1" ]; then
            ...
            /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/mnt_${1}.rrd N:${C_USED}:${C_FREE} 2>> /tmp/rrdgraphs-error.log
            break
        fi
        i=$((i+1))
        counter=MOUNT${i}
...
while [ "${1}" != "" ]; do
    ...
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/mnt_${CLEAN_NAME}.rrd N:${C_USED}:${C_FREE} 2>> /tmp/rrdgraphs-error.log
    shift 3
done
}
...
if [ $RUN_TMP -eq 1 ]; then 
    ...
    T2=0;
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/cpu_temp.rrd N:$T1:$T2 2>> /tmp/rrdgraphs-error.log 2>> /tmp/rrdgraphs-error.log
fi

# CPU frequency
if [ $RUN_FRQ -eq 1 ]; then 
    F=`sysctl -n dev.cpu.0.freq | tr -d "\n"`;
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/cpu_freq.rrd N:$F:0 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_PRO -eq 1 ]; then 
    ...
    CREATE_PVARS ${NP}
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/processes.rrd N:$total:$running:$sleeping:$waiting:$starting:$stopped:$zombie 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_CPU -eq 1 ]; then 
    ...
    CREATE_CVARS ${CP}
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/cpu.rrd N:$user:$nice:$system:$interrupt:$idle 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_MEM -eq 1 ]; then 
    ...
    CREATE_MVARS ${MM}
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/memory.rrd N:$active:$inact:$wired:$cache:$buf:$free:$swaptotal:$swapused 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_ARC -eq 1 ]; then 
    ...
    CREATE_AVARS ${ARC}
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/zfs_arc.rrd N:$AVARS 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_UPS -eq 1 ]; then 
    ...
    CREATE_UPSVARS ${CMD}
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/ups.rrd N:$charge:$load:$bvoltage:$ivoltage:$runtime:$OL:$OF:$OB:$CG 2>> /tmp/rrdgraphs-error.log
fi
...
if [ $RUN_LAT -eq 1 ]; then 
    ...
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/latency.rrd N:$PG 2>> /tmp/rrdgraphs-error.log
fi

# Uptime
if [ $RUN_UPT -eq 1 ]; then 
    UT=`echo -e "$TOP" | awk '/averages:/ {gsub("[,+:]", " "); print $10*24*60+$11*60+$12; exit}'`
    /usr/local/bin/rrdtool update $STORAGE_PATH/rrd/uptime.rrd N:$UT 2>> /tmp/rrdgraphs-error.log
fi
