...
if [ "$RUN_DUS" -eq 1 ]; then
	...
	echo "$mount" | while IFS=$'\t' read -r -a onerow ; do CREATE_MOUNTS_CMD "${onerow[0]}" "${onerow[1]}" "${onerow[2]}" ; done
	echo "$pool" | while IFS=$'\t' read -r -a onerow ; do CREATE_POOLS_CMD "${onerow[0]}" "${onerow[1]}" "${onerow[2]}" ; done
fi
#	Memory
